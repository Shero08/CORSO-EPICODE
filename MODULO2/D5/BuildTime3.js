...
function searchJob(place, job){
  const result = [];
  let counter = 0;
  for(let i = 0; i < jobs.length; i++){
    if(jobs[i].location.toUpperCase().includes(place) && jobs[i].title.toUpperCase().includes(job) || jobs[i].location.toLowerCase().includes(place) && jobs[i].title.toLowerCase().includes(job)){
      result.push(jobs[i].location, jobs[i].title);
      counter++;
      console.log(`Risultati trovati: ${counter}`);
      return result;
    }
  }
...
const search = searchJob('us', 'software');
